    ...
    public function store(Request $request)
    {
       $validated =  $request->validate([
        ...
        if($request->file('image')) {
            ...
            $image_name = $request->title.'-'.now()->timestamp.'.'.$extension;
            $request->file('image')->storeAs('cover', $image_name);
        }
    ...
    public function edit($slug)
{
    ...
    public function search(Request $request)
    {
        ...
        $books = Book::where('book_code', 'like', '%' . $search . '%')->get();
...
{
    $books = Book::orderBy($field, $order)->paginate(10);
